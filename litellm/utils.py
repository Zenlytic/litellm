...
def function_setup(  # noqa: PLR0915
    ...
    try:
        ...
        if (
            ...
            callback_list = list(
                set(litellm.input_callback + litellm.success_callback + litellm.failure_callback)  # type: ignore
            )
...
def create_pretrained_tokenizer(identifier: str, revision="main", auth_token: Optional[str] = None):
    ...
    try:
        tokenizer = Tokenizer.from_pretrained(identifier, revision=revision, auth_token=auth_token)  # type: ignore
    except Exception as e:
...
def _get_model_region(custom_llm_provider: str, litellm_params: LiteLLM_Params) -> Optional[str]:
    ...
    if custom_llm_provider == "azure":
        azure_deployment_region = (
            litellm_params.azure_deployment_region
            or litellm.azure_deployment_region
            or get_secret("AZURE_DEPLOYMENT_REGION")
        )
...
def _get_model_info_helper(model: str, custom_llm_provider: Optional[str] = None) -> ModelInfoBase:  # noqa: PLR0915
    """
...
class ProviderConfigManager:
    @staticmethod
    def get_provider_chat_config(model: str, provider: LlmProviders) -> Optional[BaseConfig]:  # noqa: PLR0915
        """
        ...
        elif provider == LlmProviders.OPENAI and litellm.OpenAIGPT5Config.is_model_gpt_5_model(model=model):
